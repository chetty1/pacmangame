                ...
                if(faces[i]->four)      //if quad
                {
                        ...
                        glBegin(GL_QUADS);
                            ...
                            if(istexture && materials[faces[i]->mat]->texture!=-1)  //if there are textures
                                        glTexCoord2f(texturecoordinate[faces[i]->texcoord[0]-1]->v,texturecoordinate[faces[i]->texcoord[0]-1]->u);      //set the texture coorinate
...
Image* img =loadBMP(filename);
        ...
        glBindTexture(GL_TEXTURE_2D,num);
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, img->width, img->height, 0, GL_RGB, GL_UNSIGNED_BYTE, img->pixels);
